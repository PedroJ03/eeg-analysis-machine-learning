...
def process_single_subject(mat_file_path):
    ...
    try:
        ...
        # --- ESTO ES PARA HACER LA CLASIFICACION EN DOS CLASES --- #
        #mask = np.where((level_changes[:, 1] == 0.00000000e+00) | (level_changes[:, 1] == 3.00000000e+00))
        #level_changes = level_changes[mask]
        ...
        # --- ESTO ES PARA HACERLO CON DOS CLASES 0 Y 3 ---#

        mask = np.where((level_changes[:, 1] == 0.00000000e+00) | (level_changes[:, 1] == 3.00000000e+00))[0]
        durations = np.diff(lista_onsets)[mask]
        print("durations", durations)

        #lista_onsets = lista_onsets[:-1]
        lista_onsets = np.array(lista_onsets)[mask]
        level_changes = level_changes[mask]
        ...
        for idx, ev in enumerate(events):
            ...
            for start in range(sample_idx, end_idx - samples_per_window, samples_per_window):
                ...
                X.append(segment)
                y.append(event_code-1) # esta linea se comenta cuando se hace el de 4 clases con 0-1 y 2-3, o el de 2 clases, 0 y 3 creo
                #y.append(0 if event_code-1 < 2 else 1) # esta linea se descomenta cuando es para 0-1 y 2-3
        X = np.array(X)
        y = np.array(y)
