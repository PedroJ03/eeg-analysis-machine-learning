...
def process_single_subject(mat_file_path):
    ...
    try:
        ...
        # --- NUEVO: TU LÓGICA DE REINICIO DE TRIALS (Bloques enteros) ---
        ...
        # 1. Calculamos los onsets y duraciones de TODOS los bloques primero
        lista_onsets_all = []
        for timestamp in primeras_ocurrencias_tiempo:
            indice_mas_cercano = np.abs(data_lsl - timestamp).argmin()
            lista_onsets_all.append(indice_mas_cercano / sfreq)

        ultimo_elemento = levels_array[-1, 0] 
        indice_ultimo = np.abs(data_lsl - ultimo_elemento).argmin()
        lista_onsets_all.append(indice_ultimo / sfreq)
        
        durations_all = np.diff(lista_onsets_all)
        lista_onsets_all = np.array(lista_onsets_all[:-1])

        # 2. AHORA filtramos solamente para quedarnos con el Nivel 0 y el Nivel 3
        mask_03 = np.where((level_changes_all[:, 1] == 0) | (level_changes_all[:, 1] == 3))[0]
        
        lista_onsets = lista_onsets_all[mask_03]
        durations = durations_all[mask_03]
        descriptions = level_changes_all[mask_03, 1].astype(str)
        ...
        X = []
        y = []
        groups_list = [] # --- NUEVO: Lista para los grupos ---
        conteo_niveles = {} # --- NUEVO: Diccionario contador ---

        for idx, ev in enumerate(events):
            sample_idx = ev[0]
            event_code = ev[2] # MNE mapea '0.0' y '3.0' a enteros (ej: 1 y 2)

            # --- NUEVO: Calcular fin del bloque usando la duración exacta ---
            ...
            # --- NUEVO: Asignación de Grupo (0, 1, 0...) ---
            if event_code not in conteo_niveles:
                conteo_niveles[event_code] = 0
                
            grupo_actual = conteo_niveles[event_code] % 2
            ...
            for start in range(sample_idx, end_idx - samples_per_window, samples_per_window):
                ...
                X.append(segment)
                y.append(event_code) # Guardamos el ID del evento de MNE
        ...
        X = np.array(X)
        y = np.array(y)
        groups = np.array(groups_list) # --- NUEVO: Convertir a numpy array ---
...
# --- FORMA EN QUE SE OBTIENEN LOS FEATURES --- #        
        ...
        train_model_result = trainModel(X_features, y, groups) # --- NUEVO: Pasar los grupos a la función de entrenamiento ---
