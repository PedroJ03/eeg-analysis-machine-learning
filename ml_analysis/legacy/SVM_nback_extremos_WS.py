...
# -------------------- ENTRENAMIENTO WITHIN-SUBJECT (SVM EXTREMOS 0 vs 3) -------------------- #
...
for path in subject_paths:
    ...
    # --- Filtrar solo clases 0 y 3 ---
    mask = np.isin(y, [0, 3])
    X_df = X_df[mask]
    y = y[mask]
    y = np.where(y == 0, 0, 1)  # 0-back → 0, 3-back → 1
    ...
    print(f"\n🚀 Entrenando SVM para {subj_name} (extremos 0 vs 3)...")
    acc, y_test, y_pred = trainModel_SVM(X, y, class_labels=[0, 1])
...
if all_accuracies:
    print("\n📈 RESULTADOS GLOBALES SVM (extremos 0 vs 3)")
    ...
    print(pd.DataFrame(cm_global, index=["Real 0", "Real 3"], columns=["Pred 0", "Pred 3"]))
else:
