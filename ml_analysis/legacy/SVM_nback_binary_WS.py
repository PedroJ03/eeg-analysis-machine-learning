...
# -------------------- ENTRENAMIENTO WITHIN-SUBJECT (SVM BINARIO 0–1 vs 2–3) -------------------- #
...
for path in subject_paths:
    ...
    # --- Reetiquetar: 0–1 → 0 (bajo esfuerzo), 2–3 → 1 (alto esfuerzo) ---
    ...
    if X_df.empty or len(np.unique(y)) < 2:
        print(f"⚠️ {subj_name}: datos insuficientes o sin ambas clases, se omite.")
        continue

    X = X_df.values
    total_epochs += len(y)

    #print(f"\n🚀 Entrenando SVM para {subj_name} (binario 0–1 vs 2–3)...")
...
if all_accuracies:
    print("\n📈 RESULTADOS GLOBALES SVM (binario 0–1 vs 2–3)")
    ...
    print(pd.DataFrame(cm_global, index=["Real bajo", "Real alto"], columns=["Pred bajo", "Pred alto"]))
else:
