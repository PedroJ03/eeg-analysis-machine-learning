...
print("Distribución de clases:", np.bincount(y.astype(int)))

# --- Entrenamiento ---
X_train, X_test, y_train, y_test = train_test_split(
    X_df.drop(columns=['subject']), y, test_size=0.2, stratify=y, random_state=42
)
...
report_df = pd.DataFrame(classification_report(
    y_test, y_pred, target_names=["0-back","1-back","2-back","3-back"], output_dict=True
)).transpose()
print(report_df.round(2))

# Matriz de confusión
cm = confusion_matrix(y_test, y_pred)
cm_df = pd.DataFrame(cm, index=["Real 0","Real 1","Real 2","Real 3"],
...
plt.title("Distribución de ensayos por nivel N-Back")
plt.xlabel("Nivel N-Back")
...
plt.figure(figsize=(6,4))
sns.boxplot(x=y, y=X_df['theta'], palette="Set2")
plt.title("Distribución de potencia theta por nivel N-Back")
plt.xlabel("Nivel N-Back")
...
report = classification_report(y_test, y_pred, target_names=["0-back","1-back","2-back","3-back"], output_dict=True)
recalls = [report[c]["recall"] for c in ["0-back","1-back","2-back","3-back"]]

plt.figure(figsize=(6,4))
sns.barplot(x=["0-back","1-back","2-back","3-back"], y=recalls, palette="rocket")
