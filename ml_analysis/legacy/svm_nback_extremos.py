...
# Filtrar solo trials de nivel 0 y 3
mask_extremos = (y == 0) | (y == 3)
X_extremos = X_df[mask_extremos].drop(columns=['subject'], errors='ignore')
y_extremos = y[mask_extremos]

# Reasignar etiquetas: 0-back → 0 (Bajo), 3-back → 1 (Alto)
y_extremos = np.where(y_extremos == 0, 0, 1)

print("Dataset extremos:", X_extremos.shape)
print("Distribución de clases (0=Bajo, 1=Alto):", np.bincount(y_extremos))


X_train, X_test, y_train, y_test = train_test_split(
    X_extremos, y_extremos,
    test_size=0.2, stratify=y_extremos, random_state=42
)
...
cm_df = pd.DataFrame(cm, index=["Real Bajo","Real Alto"],
                     columns=["Pred Bajo","Pred Alto"])
print("\n📉 Matriz de confusión (binaria)")
print(cm_df)

print("\n📊 Reporte de clasificación (binario)")
print(classification_report(y_test, y_pred, target_names=["Bajo workload","Alto workload"]))

'''
# --- Distribución de clases ---
plt.figure(figsize=(6,4))
sns.countplot(x=y_extremos, palette="viridis")
...
plt.figure(figsize=(6,4))
sns.boxplot(x=y_extremos, y=X_df['theta'], palette="Set2")
