...
def process_subject(path, debug=False):
    """Extrae características y etiquetas solo de niveles 0 y 3."""
    ...
    while i < len(exp_codes):
        ...
        # Solo usamos niveles 0 y 3
        if nback_level not in [0, 3]:
            i += 4
            continue
        ...
        if len(segment) > 0:
            ...
            X.append(feats)
            y_bin = 0 if nback_level == 0 else 1  # 0-back→0, 3-back→1
            y.append(y_bin)

        i += 4

    if debug:
        print(f"{path.split('/')[-1]} → {len(y)} trials 0/3-back extraídos")

    return pd.DataFrame(X), np.array(y)


# -------------------- ENTRENAMIENTO WITHIN-SUBJECT (RANDOM FOREST EXTREMOS 0 vs 3) -------------------- #
...
for path in subject_paths:
    ...
    if X_df.empty or len(np.unique(y)) < 2:
        print(f"⚠️ {subj_name}: sin suficientes datos 0/3-back, se omite.")
        continue
    ...
    print(f"✅ {subj_name}: precisión media (5-fold, extremos 0 vs 3) = {subj_acc:.3f}")

# -------------------- RESULTADOS FINALES -------------------- #

if all_accuracies:
    print("\n📈 RESULTADOS GLOBALES (Within-subject, extremos 0 vs 3)")
    ...
    print("\n🧠 Matriz de confusión global (0-back vs 3-back):")
    print(pd.DataFrame(cm, index=["Real 0", "Real 3"], columns=["Pred 0", "Pred 3"]))
else:
