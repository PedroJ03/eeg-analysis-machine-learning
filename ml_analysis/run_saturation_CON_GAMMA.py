        ...
        for train_idx, test_idx in split_iterator:
            ...
            if isinstance(clf, GridSearchCV):
                clf.fit(X_train, y_train,
                        groups=g_subj[train_idx] if config_type == 'window' else None)
            else:
                clf.fit(X_train, y_train)
...
def load_and_extract(subject_paths, groups_str):
    """Load data and extract features for both configs."""
    cache = {
        'trial':  {'X': [], 'y': [], 'g': []},
        'window': {'X': [], 'y': [], 'g': []}
    }
...
def run_saturation_analysis():
    ...
    for step_idx, step in enumerate(PROGRESSIVE_STEPS):
        ...
        # Quickly check if ALL combinations for this group are already evaluated
        ...
        if not need_calc:
            print("  [INFO] Todo calculado previamente. Leyendo cache...")
        ...
        for cfg in configs:
            if cache[cfg]['X']:
                ...
                break
        n_features_per_step.append(n_feats)
        print(f"  Dimension del vector de features: {n_feats}")
...
def generate_plots(df, n_features_per_step, output_dir):
    ...
    ax.set_ylabel('Delta Accuracy (puntos porcentuales)', fontsize=11)
